...
- **M3U tuner:** add the Personal Cable TV M3U URL in Jellyfin under Dashboard → Live TV → Tuner Devices → Add Tuner Device → M3U Tuner.
- **XMLTV guide:** add the Personal Cable TV XMLTV URL in Jellyfin under Dashboard → Live TV → TV Guide Data Providers → Add → XMLTV.

After saving both in Jellyfin, use **Refresh Guide Data**, then open **Live TV → Guide** to confirm the channels and schedule appear.

## v0.4.7 cleanup

- Removed the obsolete **Scan Test Media** control.
- Removed the obsolete **Legacy Metadata Import** control.
- Expanded Help & Support with clearer Jellyfin M3U/XMLTV setup instructions.
- Full Library Mode remains the normal Jellyfin connection and sync method.
